...
const log = (arg: unknown) => {
	if (env.NODE_ENV === "production") {
		...
		// TODO: Add proper logging for prod (probably sentry)
		console.log(arg)
	}
}
...
export async function POST(request: Request) {
	...
	const wpBodyResult = await tryPromise(wpResponse.value.json() as Promise<unknown>)
	...
	if (!wpResponse.value.ok) {
		const wpParsedBodyResult = WpErrorResponseSchema.safeParse(wpBodyResult.value)
	...
	const wpParsedBodyResult = WpSuccessResponseSchema.safeParse(wpBodyResult.value)
