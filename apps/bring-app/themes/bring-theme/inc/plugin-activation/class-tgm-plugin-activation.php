	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			add_filter(
				...
				[$this, "overload_textdomain_mofile"],
				10,
				2,
			);
		...
		public function init() {
			...
			if (
				true !==
				apply_filters("tgmpa_load", is_admin() && !defined("DOING_AJAX"))
			) {
				return;
			}
		...
		public function load_textdomain() {
			...
			if (
				false !== strpos(__FILE__, WP_PLUGIN_DIR) ||
				false !== strpos(__FILE__, WPMU_PLUGIN_DIR)
			) {
				// Plugin, we'll need to adjust the file name.
				add_action(
					"load_textdomain_mofile",
					[$this, "correct_plugin_mofile"],
					10,
					2,
				);
		...
		public function overload_textdomain_mofile($mofile, $domain) {
			...
			if (
				"tgmpa" !== $domain ||
				false === strpos($mofile, WP_LANG_DIR) ||
				@is_readable($mofile)
			) {
				return $mofile;
			}
		...
		public function admin_init() {
			...
			if (
				isset($_REQUEST["tab"]) &&
				"plugin-information" === $_REQUEST["tab"]
			) {
    	...
    	echo wp_kses_post($this->message);
				...
				<form id="tgmpa-plugins" action="" method="post">
					<input type="hidden" name="tgmpa-page" value="<?php echo esc_attr(
     	$this->menu,
     ); ?>" />
					<input type="hidden" name="plugin_status" value="<?php echo esc_attr(
     	$plugin_table->view_context,
     ); ?>" />
		...
		protected function do_plugin_install() {
			...
			if (
				(isset($_GET["tgmpa-install"]) &&
					"install-plugin" === $_GET["tgmpa-install"]) ||
				(isset($_GET["tgmpa-update"]) &&
					"update-plugin" === $_GET["tgmpa-update"])
			) {
				...
				$api =
					"repo" === $this->plugins[$slug]["source_type"]
						? $this->get_plugins_api($slug)
						: null;
				...
				$title =
					"update" === $install_type
						? $this->strings["updating"]
						: $this->strings["installing"];
				$skin_args = [
					"type" =>
						"bundled" !== $this->plugins[$slug]["source_type"]
							? "web"
							: "upload",
				...
				if ($this->is_automatic && !$this->is_plugin_active($slug)) {
					...
					if (
						false ===
						$this->activate_single_plugin($plugin_activate, $slug, true)
					) {
		...
		public function inject_update_info($plugins) {
			...
			foreach ($plugins as $slug => $plugin) {
				...
				if (
					empty($repo_updates->response[$file_path]->url) &&
					!empty($plugin["external_url"])
				) {
		...
		public function maybe_adjust_source_dir(
			...
			$source_files = array_keys(
				$GLOBALS["wp_filesystem"]->dirlist($remote_source),
			);
			if (
				1 === count($source_files) &&
				false === $GLOBALS["wp_filesystem"]->is_dir($source)
			) {
				return $source;
			}
			...
			if (
				false === $upgrader->bulk &&
				!empty($upgrader->skin->options["extra"]["slug"])
			) {
				...
				foreach ($this->plugins as $slug => $plugin) {
					if (
						...
						$plugin["name"] ===
							$upgrader->skin->plugin_names[$upgrader->skin->i]
					) {
						$desired_slug = $slug;
						break;
					}
			...
			if (!empty($desired_slug)) {
				$subdir_name = untrailingslashit(
					str_replace(trailingslashit($remote_source), "", $source),
				);
		...
		public function notices() {
			...
			if (
				...
				!current_user_can(
					apply_filters("tgmpa_show_admin_notice_capability", "publish_posts"),
				)
			) {
				return;
			}
			...
			foreach ($this->plugins as $slug => $plugin) {
				if (
					$this->is_plugin_active($slug) &&
					false === $this->does_plugin_have_update($slug)
				) {
					continue;
				}
				...
				if (!$this->is_plugin_installed($slug)) {
					...
					if (
						$this->does_plugin_require_update($slug) ||
						false !== $this->does_plugin_have_update($slug)
					) {
			...
			if (!empty($message) || $total_required_action_count > 0) {
				...
				$line_template =
					'<span style="display: block; margin: 0.5em 0.5em 0 0; clear: both;">%s</span>' .
					"\n";
		...
		protected function create_user_action_links_for_notice(
			...
			if (!empty($action_links)) {
				...
				return apply_filters(
					"tgmpa_notice_rendered_action_links",
					$action_links,
				);
			} else {
				return "";
		...
		public function dismiss() {
			if (
				isset($_GET["tgmpa-dismiss"]) &&
				check_admin_referer("tgmpa-dismiss-" . get_current_user_id())
			) {
				update_user_meta(
					get_current_user_id(),
					"tgmpa_dismissed_notice_" . $this->id,
		...
		public function register($plugin) {
			...
			if (
				...
				isset($this->plugins[$plugin["slug"]])
			) {
				return;
			}
			...
			$plugin["force_activation"] = TGMPA_Utils::validate_bool(
				$plugin["force_activation"],
			);
			$plugin["force_deactivation"] = TGMPA_Utils::validate_bool(
		...
		public function populate_file_path($plugin_slug = "") {
			if (
				!empty($plugin_slug) &&
				is_string($plugin_slug) &&
				isset($this->plugins[$plugin_slug])
			) {
				$this->plugins[$plugin_slug][
					"file_path"
				] = $this->_get_plugin_basename_from_slug($plugin_slug);
			} else {
				// Add file_path key for all plugins.
				foreach ($this->plugins as $slug => $values) {
					$this->plugins[$slug][
						"file_path"
					] = $this->_get_plugin_basename_from_slug($slug);
				}
			}
		}
		...
		public function get_installed_version($slug) {
			...
			if (
				!empty(
					$installed_plugins[$this->plugins[$slug]["file_path"]]["Version"]
				)
			) {
				return $installed_plugins[$this->plugins[$slug]["file_path"]][
					"Version"
				];
			}
		...
		public function does_plugin_have_update($slug) {
			...
			if (
				isset(
					$repo_updates->response[$this->plugins[$slug]["file_path"]]
						->new_version,
				)
			) {
				return $repo_updates->response[$this->plugins[$slug]["file_path"]]
					->new_version;
			}
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function _gather_plugin_data() {
			...
			foreach ($plugins[$this->view_context] as $slug => $plugin) {
				...
				$table_data[$i]["source"] = $this->get_plugin_source_type_text(
					$plugin["source_type"],
				);
				$table_data[$i]["type"] = $this->get_plugin_advise_type_text(
					$plugin["required"],
				);
				$table_data[$i]["status"] = $this->get_plugin_status_text($slug);
				$table_data[$i][
					"installed_version"
				] = $this->tgmpa->get_installed_version($slug);
				$table_data[$i]["minimum_version"] = $plugin["version"];
				$table_data[$i][
					"available_version"
				] = $this->tgmpa->does_plugin_have_update($slug);
				...
				if (!empty($upgrade_notice)) {
					...
					add_action(
						"tgmpa_after_plugin_row_{$slug}",
						[$this, "wp_plugin_update_row"],
						10,
						2,
					);
				}

				$table_data[$i] = apply_filters(
					"tgmpa_table_data_item",
					$table_data[$i],
					$plugin,
				);
		...
		public function column_version($item) {
			...
			if ($this->tgmpa->is_plugin_installed($item["slug"])) {
				...
				if (
					...
					$this->tgmpa->does_plugin_require_update($item["slug"])
				) {
		...
		public function single_row($item) {
			echo '<tr class="' .
				esc_attr("tgmpa-type-" . strtolower($item["type"])) .
				'">';
			 ...
			 */
			do_action(
				"tgmpa_after_plugin_row_{$item["slug"]}",
				$item["slug"],
				$item,
				$this->view_context,
			);
		...
		public function get_bulk_actions() {
			...
			if (
				"update" !== $this->view_context &&
				"activate" !== $this->view_context
			) {
				if (current_user_can("install_plugins")) {
		...
		public function process_bulk_actions() {
			...
			if (
				"tgmpa-bulk-install" === $this->current_action() ||
				...
				foreach ($plugins_to_install as $key => $slug) {
					...
					if (
						...
						true === $this->tgmpa->is_plugin_installed($slug)
					) {
						unset($plugins_to_install[$key]);
					}
					...
					if (
						"update" === $install_type &&
						false === $this->tgmpa->is_plugin_updatetable($slug)
					) {
						unset($plugins_to_install[$key]);
					}
				}
				...
				add_filter(
					"upgrader_source_selection",
					[$this->tgmpa, "maybe_adjust_source_dir"],
					1,
					3,
				);
		...
		public function prepare_items() {
			...
			$this->items = apply_filters(
				"tgmpa_table_data_items",
				$this->_gather_plugin_data(),
			);
		}
			...
			if (!class_exists("TGMPA_Bulk_Installer")) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function activate_strings() {
						$this->strings["activation_failed"] = __(
							"Plugin activation failed.",
							"tgmpa",
						);
						$this->strings["activation_success"] = __(
						...
						if ($this->tgmpa->is_automatic) {
							...
							if (!is_plugin_active($plugin_info)) {
								...
								if (is_wp_error($activate)) {
									...
									$this->strings["process_success"] .=
										$this->strings["activation_failed"];
								} else {
									$this->strings["process_success"] .=
										$this->strings["activation_success"];
								}
							}
						}
